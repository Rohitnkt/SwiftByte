...
<%
    ...
    // Unique cuisines for the "What's on your mind?" strip
    ...
    // Emoji map for popular cuisines
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  body{
    font-family:'ProximaNova','Inter',system-ui,-apple-system,Segoe UI,Roboto,sans-serif;
  ...
  .cuisine-circle{
    ...
    background:linear-gradient(135deg,#fff4ec 0%,#ffe0cc 100%);
    display:flex;align-items:center;justify-content:center;font-size:56px;
    margin-bottom:8px;box-shadow:inset 0 -4px 12px rgba(0,0,0,.04);
  }
  ...
  .card-title{font-size:18px;font-weight:700;color:var(--text);margin-bottom:6px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
...
<nav class="navbar">
  <div class="nav-container">
    <a class="logo" href="restaurants">
      <span class="logo-icon">S</span> SwiftByte
    </a>
...
<section class="container">
  ...
  <div class="cuisine-strip">
    ...
    <%
      ...
      if (shown == 0) {
    %>
      <div class="cuisine-item"><div class="cuisine-circle">🍽️</div><div class="cuisine-name">No cuisines yet</div></div>
    <% } %>
  </div>
  ...
  <div class="restaurant-grid">
    ...
    <%
        ...
        for (Restaurant r : restaurants) {
            ...
            String promo = (r.getPromoOffer() == null || r.getPromoOffer().isEmpty()) ? "" : r.getPromoOffer();
    %>
      <a class="card" href="menu?restaurantId=<%= r.getRestaurantId() %>">
        <div class="card-image">
          ...
          <% if (r.isTopChain()) { %>
            <span class="top-chain-badge">★ Top Chain</span>
          <% } %>
          <span class="status-pill <%= open ? "status-open" : "status-closed" %>">
            <%= open ? "Open" : "Closed" %>
          </span>
          <% if (!promo.isEmpty()) { %>
        ...
        <div class="card-body">
          <div class="card-title"><%= r.getRestaurantName() %></div>
...
<footer class="footer">
  © 2026 SwiftByte. Crafted for premium food delivery.
</footer>
